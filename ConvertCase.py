...
def parse_casing(content):
    ...
    camel_case_parts = []
    match = re.match('^[^%s]+' % RE_UNICODE_UPPERCASE, content)
    if match:
        camel_case_parts.append(match.group(0))
    for match in re.finditer(RE_CAMELCASE_SPLITTER, content):
